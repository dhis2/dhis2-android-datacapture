...
public class FinOctYearPeriodFilter extends PeriodFilter {
    ...
    private static DateTime fixStartDate(DateTime startDate) {
        ...
        if (month <= 9) {
            return startDate.withMonthOfYear(DateTimeConstants.OCTOBER).withDayOfMonth(01).withYear(
                    startDate.getYear() - 1);
        } else {
            return startDate.withMonthOfYear(DateTimeConstants.OCTOBER).withDayOfMonth(01);
        }
    }
    ...
    private static DateTime fixEndDate(DateTime endDate) {
        ...
        if (month <= 9) {
            ...
            return endDate.withMonthOfYear(DateTimeConstants.SEPTEMBER).withDayOfMonth(30).withYear(endDate.getYear()+1);
        }
    }
